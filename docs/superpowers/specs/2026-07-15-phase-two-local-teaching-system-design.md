...
`runtime/manifest.json` 记录 `schema_version: 1`、源数据 SHA-256、索引状态和最后成功启动时间。篮子与教案分别存为 `basket.json` 和 `lesson-plans/<id>.json`，均包含单调递增的整数 `revision`。更新请求必须携带当前 revision；不匹配返回 `409 REVISION_CONFLICT` 和服务端最新对象。单进程内写入使用互斥锁，同一文件不并发写。
...
- `se_knowledge.json`：`{chapters:[{num,title,page_start,page_end}], sections:[{chapter,section,section_title,page_start,page_end,definitions,keywords}]}`；章节号为1至16，范围闭合且不得越出1至398。
- `se_keyword_index.json`：`{keywords:[{term,freq,occurrences:[{page,pos,context}]}], page_terms}`；`pos` 是对应页面 OCR 字符串的 Unicode code point 索引。
...
- `chapter1_workspace.json`：第一章高精度页面、出现项和插图采用第一期规格中的 schema；只在扫描14至39页启用坐标回跳。
- `chapter1_resources.json` 与 `se_external_resources.json`：资源至少包含稳定ID、标题、平台、URL、关联概念或章节以及校验状态；重复 URL 规范化后只保留校验时间最新项。
...
确定性组织规则：取评分最高且分数达到查询词最高可能标题分的20%的最多5个片段；按页面顺序合并同义重复句。若问题包含“什么是/定义”，优先抽取含“是、指、称为、定义”且命中查询词的完整句；包含“区别/比较”时按命中实体分组列出各组原句；包含“步骤/过程/阶段”时按原文序号或句序列出；其他问题给出不超过3条摘录式要点。不得补充片段中未出现的因果、数字、作者或结论。最高结果低于阈值或有效片段少于1条时拒答。
...
- `GET /api/health` -> `{status:"ready"|"indexing"|"degraded", index:{processed,total,percent}, data_version, capabilities:{docx,pptx}}`。
...
稳定错误码至少包括 `SERVICE_INDEXING`、`DATA_SCHEMA_INVALID`、`INDEX_BUILD_FAILED`、`VALIDATION_ERROR`、`PAGE_OUT_OF_RANGE`、`EVIDENCE_NOT_FOUND`、`REVISION_CONFLICT`、`LESSON_INCOMPLETE`、`EXPORT_DEPENDENCY_MISSING`、`EXPORT_FAILED`、`STORAGE_UNAVAILABLE` 和 `NOT_FOUND`。请求体最大1 MB；所有路径和文件名由服务端生成，客户端不得传入文件系统路径。
...
- 检索测试：提交 `tests/fixtures/retrieval-gold.json`，含至少12个问题、每题允许页集合和至少2个拒答案例；每题前5条至少一条命中允许页，整体命中率100%。
- 回答测试：每个事实段至少一个有效引用；无结果问题不得编造答案。
- API测试：正常、空输入、超长输入、非法章节、未知产物和路径穿越。
